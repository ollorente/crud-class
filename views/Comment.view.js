...
createClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/comments`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        data.data.forEach(e => {
            ...
            <div class="media bg-white" style="border-bottom:solid 1px #fff;">
                <a href="/comments/${e.username}"><img src="https://www.gravatar.com/avatar/${e.gravatar}?s=160&r=pg" class="mr-3" alt="+${e.username}" style="width:120px; height:160px; object-fit: cover; position: center center;"></a>
...
GetOneClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/comments/ollorente`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
    })
    .then(res => res.json())
...
UpdateClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/comments/etniastudio`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'jwt': config.Token
...
RemoveClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/comments/1`, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'jwt': config.Token
