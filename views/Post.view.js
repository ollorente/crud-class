...
createClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/posts`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        data.data.forEach(e => {
            ...
            <div class="media bg-white" style="border-bottom:solid 1px #fff;">
                <a href="/posts/${e.username}"><img src="https://www.gravatar.com/avatar/${e.gravatar}?s=160&r=pg" class="mr-3" alt="+${e.username}" style="width:120px; height:160px; object-fit: cover; position: center center;"></a>
...
GetOneClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/posts/ollorente`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
    })
    .then(res => res.json())
...
UpdateClick.addEventListener('click', (e) => {
    ...
    fetch(`${config._URL}${config._Version}/posts/etniastudio`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'jwt': config.Token
